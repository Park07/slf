...
def organize_test_files():
    """Organize full_test_ and config_ files into 12/Aug directory structure"""

    # Create base directory
    base_dir = "12/Aug"
    os.makedirs(base_dir, exist_ok=True)

    # Get all files that start with full_test_ or config_
    test_files = glob.glob("full_test_*") + glob.glob("config_*")
    ...
    for file in test_files:
        ...
        # config_small_dense_4_query_dense_4_1.graph.json -> small_dense_4_query_dense_4_1

        if file.startswith("full_test_"):
            # Remove "full_test_" prefix and file extension
            identifier = file[10:]  # Remove "full_test_"
            if identifier.endswith(".graph.json"):
                identifier = identifier[:-11]  # Remove ".graph.json"
            elif identifier.endswith(".graph"):
                identifier = identifier[:-6]   # Remove ".graph"

        elif file.startswith("config_"):
            # Remove "config_" prefix and file extension
            identifier = file[7:]   # Remove "config_"
            if identifier.endswith(".json"):
                identifier = identifier[:-5]   # Remove ".json"

...
def parse_log_file(log_file):
    ...
    try:
        ...
        if mappings == 0 and time_ms is None:
            timeout = True
...
def main():
    ...
    for (category, density, threads), data in sorted(results.items()):
        ...
        time_str = f"{avg_time:.1f}" if avg_time is not None else "N/A"
        print(f"{category},{density},{threads},{avg_mappings:.0f},{time_str},{timeout_rate:.2f},{len(data)}")
